...
public class RyaDetailsFormatter {
    ...
    public String format(final RyaDetails details) {
        ...
        if(pcjDetails.isEnabled()) {
            ...
            final ImmutableList<PCJDetails> pcjs = pcjDetails.getPCJDetails();
            report.append("    PCJs:\n");
            if(pcjs.isEmpty()) {
                report.append("      No PCJs have been added yet.\n");
            } else {
                for(final PCJDetails pcj : pcjs) {
                    report.append("      ID: ").append(pcj.getId()).append("\n");
                    report.append("        Update Strategy: ").append(pcj.getUpdateStrategy()).append("\n");

                    final String lastUpdateTime = format( pcj.getLastUpdateTime() );
        ...
        report.append("    Last Update Time: ").append( format(details.getProspectorDetails().getLastUpdated()) ).append("\n");
        report.append("  Join Selectivity:\n");
        report.append("    Last Updated Time: ").append( format(details.getJoinSelectivityDetails().getLastUpdated()) ).append("\n");
     ...
     */
    private <T> String format(final Optional<T> value) {
